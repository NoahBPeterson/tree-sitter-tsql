...
- [ ] `full_column_name` — multi-part: `table.column`, `schema.table.column` (Parser L5155-L5160)
...
- [ ] `SELECT ALL` (Parser L4011)
- [ ] `SELECT DISTINCT` (Parser L4011)
- [ ] `top_clause` — `TOP (n)` (Parser L4026)
- [ ] `top_percent` — `TOP (n) PERCENT` (Parser L4030)
- [ ] `top_count` — `TOP n` without parens (Parser L4035)
- [ ] `WITH TIES` (Parser L4027)
- [ ] `INTO table_name` — `SELECT ... INTO #temp` (Parser L4015)
- [ ] `GROUP BY ALL` (Parser L4018)
- [ ] `GROUP BY ROLLUP(col, ...)` (Parser L4018)
- [ ] `GROUP BY CUBE(col, ...)` (Parser L4018)
- [ ] `GROUP BY GROUPING SETS(...)` (Parser L4019)
- [ ] `grouping_sets_item` (Parser L4019)
...
- [ ] `with_expression` (CTEs) — `WITH cte AS (SELECT ...) SELECT ...` (Parser L3955)
- [ ] `common_table_expression` — `name [(cols)] AS (select)` (Parser L3959)
- [ ] Recursive CTEs (Parser L3955)
- [ ] `select_order_by_clause` — `ORDER BY col [ASC|DESC]` (Parser L4046)
- [ ] `OFFSET n ROWS` (Parser L4048)
- [ ] `FETCH FIRST|NEXT n ROWS ONLY` (Parser L4049)
...
- [ ] `sql_union` — `UNION [ALL]` (Parser L4002)
- [ ] `sql_union` — `EXCEPT` (Parser L4002)
- [ ] `sql_union` — `INTERSECT` (Parser L4002)
...
- [ ] Multiple `table_sources` — comma-separated (implicit cross join) (Parser L4152)
- [ ] `as_table_alias` — `table AS alias` / `table alias` (Parser L4171)
- [ ] `with_table_hints` — `WITH (NOLOCK)`, etc. (Parser L4173)
- [ ] Derived tables — `(SELECT ...) AS alias` (Parser L4172)
- [ ] Table-valued functions — `dbo.fn_table(@param)` (Parser L4175)
- [ ] `LOCAL_ID` as table source — `@tablevar` (Parser L4176)
...
- [ ] `join_part` rule — attach joins to table_source (Parser L4224)
- [ ] `INNER JOIN ... ON condition` (Parser L4233)
- [ ] `LEFT [OUTER] JOIN ... ON condition` (Parser L4234)
- [ ] `RIGHT [OUTER] JOIN ... ON condition` (Parser L4234)
- [ ] `FULL [OUTER] JOIN ... ON condition` (Parser L4234)
- [ ] `CROSS JOIN` (Parser L4239)
- [ ] `CROSS APPLY` (Parser L4243)
- [ ] `OUTER APPLY` (Parser L4243)
- [ ] Join hints — `LOOP`, `HASH`, `MERGE`, `REMOTE` (Parser L4235)
...
- [ ] `table_name.* ` — qualified asterisk (Parser L4147)
...
- [ ] Block comments `/* ... */` (nestable) (Lexer L1214)
- [ ] Single-line comments `-- ...` (Lexer L1215)
